...
test_expect_success "HTTP GET empty directory" '
  URL="http://localhost:$port/btfs/$HASH_EMPTY_DIR/" &&
  echo "GET $URL" &&
...
test_expect_success "HTTP PUT file to construct a hierarchy" '
  echo "$RANDOM" >infile &&
  URL="http://localhost:$port/btfs/$HASH_EMPTY_DIR/test.txt" &&
  echo "PUT $URL" &&
...
test_expect_success "We can HTTP GET file just created" '
  URL="http://localhost:$port/btfs/$HASH/test.txt" &&
  echo "GET $URL" &&
  curl -so outfile "$URL" &&
  test_cmp infile outfile
'
...
test_expect_success "HTTP PUT file to append to existing hierarchy" '
  echo "$RANDOM" >infile2 &&
  URL="http://localhost:$port/btfs/$HASH/test/test.txt" &&
  echo "PUT $URL" &&
  ...
  HASH=$(expr "$LOCATION" : "< Location: /btfs/\(.*\)/test/test.txt")
'
...
test_expect_success "HTTP PUT to replace a directory" '
  echo "$RANDOM" >infile3 &&
  URL="http://localhost:$port/btfs/$HASH/test" &&
  echo "PUT $URL" &&
  ...
  HASH=$(expr "$LOCATION" : "< Location: /btfs/\(.*\)/test")
'
...
test_expect_success "HTTP PUT to /ipns fails" '
  ...
  URL="http://localhost:$port/btns/$PEERID/test.txt" &&
  echo "PUT $URL" &&
