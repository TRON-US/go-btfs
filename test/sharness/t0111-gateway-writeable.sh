...
test_expect_success "HTTP PUT file to construct a hierarchy" '
  echo "$RANDOM" >infile &&
  URL="http://127.0.0.1:$port/btfs/$HASH_EMPTY_DIR/test.txt" &&
  echo "PUT $URL" &&
...
test_expect_success "We can HTTP GET file just created" '
  URL="http://127.0.0.1:$port/btfs/$HASH/test.txt" &&
  echo "GET $URL" &&
  curl -so outfile "$URL" &&
  test_cmp infile outfile
'
...
test_expect_success "HTTP PUT file to append to existing hierarchy" '
  echo "$RANDOM" >infile2 &&
  URL="http://127.0.0.1:$port/btfs/$HASH/test/test.txt" &&
  echo "PUT $URL" &&
...
test_expect_success "HTTP PUT to replace a directory" '
  echo "$RANDOM" >infile3 &&
  URL="http://127.0.0.1:$port/btfs/$HASH/test" &&
  echo "PUT $URL" &&
...
test_expect_success "HTTP PUT to /ipns fails" '
  ...
  URL="http://127.0.0.1:$port/btns/$PEERID/test.txt" &&
  echo "PUT $URL" &&
